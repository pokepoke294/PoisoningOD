...
class Kettle():
    ...
    def __init__(self, args, batch_size, augmentations, setup=dict(device=torch.device('cuda'), dtype=torch.float)):
    ...
    def print_status(self):
        ...
        if self.args.ablation < 1.0:
            ...
            num_p_poisons = len(np.intersect1d(self.poison_ids.cuda().numpy(), np.array(self.sample)))
    ...
    def initialize_poison(self, initializer=None):
        ...
        if self.args.local_rank is not None:
            if DISTRIBUTED_BACKEND == 'nccl':
                ...
                init.to(device=torch.device('cuda'))
            else:
                torch.distributed.broadcast(init, src=0)
        return init
    ...
    def export_poison(self, poison_delta, path=None, mode='automl'):
        ...
        def _torch_to_PIL(image_tensor):
            ...
            image_torch_uint8 = image_denormalized.mul(255).add_(0.5).clamp_(0, 255).permute(1, 2, 0).to('cuda', torch.uint8)
            image_PIL = PIL.Image.fromarray(image_torch_uint8.cpu().numpy())
            return image_PIL
