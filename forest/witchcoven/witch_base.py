...
class _Witch():
    ...
    def _run_trial(self, victim, kettle):
        ...
        backup_poison_delta = poison_delta.grad
        ...
        for step in range(self.args.attackiter):
            ...
            # print(poison_delta)
            # print("----------------------------------")
            if poison_delta.grad == None:
              break
            for batch, example in enumerate(dataloader):
                
                loss, prediction = self._batched_step(poison_delta, poison_bounds, example, victim, kettle,backup_poison_delta)
                target_losses += loss
    ...
    def _batched_step(self, poison_delta, poison_bounds, example, victim, kettle, backup_poison_delta):
        ...
        if len(batch_positions) > 0:
            ...
            if self.args.attackoptim in ['PGD', 'GD']:
                ...
                #print(delta_slice.grad.detach().to(device=torch.device('cpu')))
                # print(poison_delta)
                # print(poison_slices)
                # print(batch_positions)
                # print('----------------------------')
                # print(len(delta_slice.grad.detach().to(device=torch.device('cpu'))))
                # print(len(poison_delta))
                # print(type(poison_delta))
                # print(type(poison_delta.grad))
                # print(type(poison_delta.grad[poison_slices]))
                #raise NotImplementedError('Unknown attack optimizer.')
                # testData = poison_delta
                # if testData.grad == None:
                #   poison_delta[poison_slices] = delta_slice.grad.detach().to(device=torch.device('cpu'))
                # else:
